...
class StockList:
    ...
    def update_price(self):
        ...
        get_query = "SELECT symbol, curr_price, day_high, day_low FROM stocks"
        ...
        for stock in output:
            ...
            update_query = "UPDATE stocks SET curr_price=%s, day_low=%s, day_high=%s WHERE symbol=%s"
            self.db.run_query(update_query, float(updated_price), float(updated_low), float(updated_high), stock["symbol"])
